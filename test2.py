...
# 分析情緒、年齡和性別的函數
def analyze_frame_A(frame, class_name, confidence_score, emotions_over_time, ages_over_time, check_time):
    try:
    ...
    if frame_count % frame_interval == 0:
        ...
        if class_1_detected and start_time_1 and (time.time() - start_time_1) > 3:
            ...
            if check_time <= 8: #超過分析超過五秒年齡性別得出結論->直接使用第五秒最後一次判斷的性別年齡結果
                result = analyze_frame_A(frame, class_name, confidence_score, emotions_over_time, ages_over_time, check_time)
            ...
            if check_time <= 8: #超過分析超過五秒年齡性別得出結論->直接使用第五秒最後一次判斷的性別年齡結果
                result1 = analyze_frame_A(frame1, class_name1, confidence_score1, emotions_over_time1, ages_over_time1, check_time)
...
if ages_over_time :
    ...
    title_text = f"Emotion Wave Over Time in Cam0 (Avg Age: {avg_age})"
else:
    title_text = "Emotion Wave Over Time"

if ages_over_time1 :
    avg_age1 = round(np.mean(ages_over_time1))
    title_text1 = f"Emotion Wave Over Time in Cam1 (Avg Age: {avg_age1})"
else:
